    ...
    public void print(){
        for(String names: this.Variablelist.keySet()){
            if(!this.Variablelist.get(names).inclass){
                this.Variablelist.get(names).offset=this.varOffset;
                System.out.println(this.Name+"."+this.Variablelist.get(names).Name+" : "+this.Variablelist.get(names).offset);
        ...
        for(String names: this.functionlist.keySet()){
            if(!this.functionlist.get(names).ghost){
                this.functionlist.get(names).offset=this.funcOffset;
                System.out.println(this.Name+"."+this.functionlist.get(names).Name+" : "+this.functionlist.get(names).offset);
                this.funcOffset+=8;
            }
        }
    }
